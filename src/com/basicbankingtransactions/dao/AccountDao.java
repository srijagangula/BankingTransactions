package com.basicbankingtransactions.dao;
...
import com.basicbankingtransactions.db.DBUtil;
...
public class AccountDao {
	...
	public Account insert(Account account) {
		...
		if(account.getPassword() == null) {
			account.setPassword(account.getMobileNumber().substring(account.getMobileNumber().length()-4));
		}
	...
	public double withdraw(Account account, double amount) throws InsufficientBalanceException, MinmumBalanceException {
		...
		else if(account.getBalance() - amount < Account.ACCOUNT_THRESHOLD_AMOUNT) {
			throw new MinmumBalanceException("MININUM BALLANCE MUST BE MAINTAINED");
		}
